...
def write_xcorr_com(tilt_dir_name, tilt_name, tilt_extension, tilt_axis_ang):
    ...
    with open(f"{tilt_dir_name}/xcorr_coarse.com", "w") as fout:
        ...
        fout.write("# or XMinAndMax and YMinAndMax  to specify a non-centered region\n")
        fout.write("#\n")
...
def track_patch_com(
    tilt_dir_name, tilt_name, pixel_nm, binval, tilt_axis_ang, dimX, dimY
):
    """
    track_patches
    """
    with open(f"{tilt_dir_name}/align_patch.com", "w") as fout:
        ...
        fout.write("#\n")
        fout.write("# Minimum fiducials total and on one surface if two surfaces\n")
        fout.write("#\n")
        ...
        fout.write(
            f"$b3dcopy -p {tilt_dir_name}/{tilt_name}_fid.xf {tilt_dir_name}/{tilt_name}.xf\n"
        )
        fout.write(
            f"$b3dcopy -p {tilt_dir_name}/{tilt_name}.tlt {tilt_dir_name}/{tilt_name}_fid.tlt\n"
        )
        fout.write("#\n")
...
def make_ali_stack_com(tilt_dir_name, tilt_name, tilt_extension, binval, dimX, dimY):
    ...
    with open(tilt_dir_name + "/" + "newst_ali.com", "w") as fout:
        fout.write(
            f"# The offset argument should be 0,0 for no offset, 0,300 to take an area\n"
        )
...
def make_tomogram(
    tilt_dir_name, tilt_name, tilt_extension, dimX, dimY, binval, thickness
):
    dimX = int(dimX)
    dimY = int(dimY)
    binval = int(binval)
    ...
    with open(tilt_dir_name + "/" + "tilt_ali.com", "w") as fout:
        ...
        fout.write(f"#\n")
        fout.write(
            f"# RADIAL specifies the frequency at which the Gaussian low pass filter begins\n"
        )
        fout.write(f"#   followed by the standard deviation of the Gaussian roll-off\n")
        fout.write(f"#\n")
        fout.write(
            f"# LOG takes the logarithm of tilt data after adding the given value\n"
        )
        fout.write(f"#\n")
