...
def update_warp_xml(
    ...
    backup_xml: bool = typer.Option(True, "--backup-xml/--no-backup-xml", help="Create a backup of the XML file before updating. Defaults to True.") # New option
):
    """
    ...
    # New backup logic
    if backup_xml:
        backup_file_path = xml_file.with_suffix(xml_file.suffix + ".bak") # e.g., file.xml.bak
        try:
    ...
    good_mask = (
        ~tomostar_data["wrpMovieName"].apply(lambda x: Path(x).stem).isin(bad_frames)
    )
